...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same score-based ordering as `run_prepare` so this marks exactly the
    articles that were written to pending_ingest.md, not a differently-sorted batch.
    """
    from prioritize import prioritize
