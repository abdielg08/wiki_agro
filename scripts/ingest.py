...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Must select the same top-N articles that `ingest` actually showed Claude —
    `find_pending()` alone returns filename order, while `ingest` selects by
    `prioritize(..., strategy="score")`. Using plain `find_pending` order here
    previously could mark a *different* unreviewed article as ingested instead
    of the one Claude actually processed, silently dropping it from the queue
    without ever entering the wiki.
    """
    from prioritize import prioritize
