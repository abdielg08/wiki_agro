...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same score-based priority order as `ingest`, so `mark-all-ingested --limit N`
    marks the same N articles that `ingest --limit N` most recently presented for review.
    """
    from prioritize import prioritize
