...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same "score" prioritization as `ingest`'s default so that
    `mark-all-ingested --limit N` marks the same N articles that `ingest
    --limit N` showed in pending_ingest.md — find_pending() alone returns
    filename order, which silently diverges from the score-ordered batch.
    """
    from prioritize import prioritize
    processed = load_processed()
    articles = article_entries(processed)
