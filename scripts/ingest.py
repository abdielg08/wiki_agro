...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same score-based prioritization as `run_prepare` (the `ingest`
    command) so the articles marked here are exactly the ones Claude Code
    was actually shown in pending_ingest.md — not an unrelated filename-sorted
    slice of the queue.
    """
    from prioritize import prioritize
