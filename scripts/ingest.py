...
def mark_all_ingested(limit: int = 0, strategy: str = "score") -> int:
    ...
    Uses the same priority ordering as `ingest` (default strategy="score") so the
    articles marked here match the ones Claude was actually shown in pending_ingest.md.
    """
    from prioritize import prioritize
