...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same priority ordering as `run_prepare` (score-based), so this
    marks the same articles that `ingest` actually showed to Claude — not an
    unrelated alphabetical slice.
    """
    from prioritize import prioritize
