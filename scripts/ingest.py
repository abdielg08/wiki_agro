...
def mark_ingested(url_or_slug: str) -> bool:
    ...
    for url, meta in processed.items():
        if not isinstance(meta, dict):
            continue  # skip non-article keys, e.g. "_gdelt_windows"
...
def mark_all_ingested(
    ...
    Must use the same selection (prioritize by score, by default) as `run_prepare`
    uses to build pending_ingest.md — otherwise this marks a different set of
    articles than the ones Claude actually reviewed, leaving reviewed articles
    stuck pending while unreviewed ones get silently marked ingested.
    """
    from prioritize import prioritize
