...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Must use the same ordering as `run_prepare` (score-based priority), since that
    is the order Claude Code actually saw and processed in pending_ingest.md.
    Sorting by filename here instead would mark a different set of articles as
    ingested than the ones actually written up in the wiki.
    """
    from prioritize import prioritize
