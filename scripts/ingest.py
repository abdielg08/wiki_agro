...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same priority ordering as `run_prepare` (the `ingest` command), so this
    marks exactly the articles that were shown to Claude — not a different filename-sorted
    subset, which previously caused unreviewed articles to be silently marked as ingested.
    """
    from prioritize import prioritize
