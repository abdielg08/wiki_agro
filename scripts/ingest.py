...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same priority ordering as `ingest` (scripts/prioritize.py), so the
    articles marked here are the same ones `ingest --limit N` wrote to
    pending_ingest.md for Claude to process — not an unrelated ordering.
    """
    from prioritize import prioritize
