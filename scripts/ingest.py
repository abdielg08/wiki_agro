...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Must select the same articles `run_prepare()` showed Claude (top-N by score),
    not filesystem/glob order — otherwise this marks different articles than the
    ones actually reviewed, silently hiding unreviewed articles as "ingested" while
    leaving reviewed ones stuck pending.
    """
    from prioritize import prioritize
