...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same default priority order ("score") as `ingest`/run_prepare, so this
    marks the same batch that was just written to pending_ingest.md instead of an
    unrelated set of articles.
    """
    from prioritize import prioritize
