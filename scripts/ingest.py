...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same score-based priority order as `run_prepare`/`ingest`, so this
    marks the same articles Claude Code was actually shown in pending_ingest.md
    (find_pending's raw filename order would otherwise pick a different set).
    """
    from prioritize import prioritize
