...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same score-based prioritization as `ingest`, so `mark-all-ingested
    --limit N` always matches the exact N articles the last `ingest --limit N`
    call wrote to pending_ingest.md (find_pending() alone returns filename-sorted
    order, which can diverge from the priority order Claude actually reviewed).
    """
    from prioritize import prioritize
