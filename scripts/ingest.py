...
def mark_ingested(url_or_slug: str) -> bool:
    ...
    for url, meta in processed.items():
        if url.startswith("_") or not isinstance(meta, dict):
            continue
...
def mark_all_ingested(limit: int = 0) -> int:
    ...
    Uses the same priority ordering as `run_prepare` (score-based) so this marks
    the exact same articles that were shown in pending_ingest.md — not a
    different set picked by filesystem order.
    """
    from prioritize import prioritize
